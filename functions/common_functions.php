                ...
                <div class='col-12 col-sm-6 col-md-4 mb-4'>
                    <div class='card shadow-sm border-light' style='border-radius: 15px;'>
                        <div class='image-wrapper' style='position: relative; padding-top: 75%;'>
                            <img src='./admin/product_images/$product_image_one' class='card-img-top img-fluid' alt='$product_title' style='position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: contain; border-radius: 15px 15px 0 0;'>

                        </div>
                        <div class='card-body'>
                            <h5 class='card-title text-truncate' style='font-weight: bold; color: #2A91E7;'>$product_title</h5>
                            <p class='card-text text-muted'>$ $product_price</p>
                            <div class='d-flex justify-content-between align-items-center'>
                                <a href='products.php?add_to_cart=$product_id' class='btn btn-primary btn-sm' style='background-color: #2A91E7; border-color: #2A91E7;'>Add To Cart</a>
                                <a href='product_details.php?product_id=$product_id' class='btn btn-outline-primary btn-sm' style='color: #2A91E7;'>View More</a>
                            </div>
                        </div>
                        <div class='card-footer text-center' style='background-color: #FFE7FA; border-radius: 0 0 15px 15px;'>
                            <div class='d-flex justify-content-center align-items-center'>
                                <span class='text-warning'>
                                    ...
                                    <!-- Repeat the SVG for full stars or partial stars -->
                                </span>
        ...
        <div class='col-md-4 mb-4'>
            <div class='card'>
                ...
                <div class='card-body'>
                    ...
                    <div class='d-flex justify-content-between align-items-center'>
                        <a href='products.php?add_to_cart=$product_id' class='btn btn-primary btn-sm'>Add to Cart</a>
                        <a href='product_details.php?product_id=$product_id' class='btn btn-secondary btn-sm'>View More</a>
                    </div>
                </div>
...
function cart($num_of_items = 1)
{
    if (isset($_GET['add_to_cart'])) {
        ...
        if ($num_of_rows > 0) {
            ...
            $update_query = "UPDATE `card_details` SET quantity = quantity + $num_of_items WHERE ip_address = '$getIpAddress' AND product_id = $getProductId";
            $update_result = mysqli_query($con, $update_query);
            echo "<script>alert('This item quantity updated in the Cart');</script>";
            echo "<script>window.open('products.php','_self');</script>";
        } else {
            // Product not in the cart, insert into the cart
            $insert_query = "INSERT INTO `card_details` (product_id, ip_address, quantity)
                             VALUES ($getProductId, '$getIpAddress', $num_of_items)
                             ON DUPLICATE KEY UPDATE quantity = quantity + $num_of_items";
